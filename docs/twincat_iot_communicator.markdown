...
{% note %}
OIDC Discovery is required. If the identity provider does not serve `/.well-known/openid-configuration` or `/.well-known/oauth-authorization-server`, the setup will fail with an error.
{% endnote %}
