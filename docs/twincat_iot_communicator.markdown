...
Scalar PLC values that are not part of a widget are also auto-discovered based on their field suffix and mapped to the appropriate platform. Every datatype gets a single entity regardless of read-only status — read-only protection is enforced at command time, not at entity creation.

| PLC datatype                     | Home Assistant platform |
| -------------------------------- | ---------------------- |
| BOOL (`bBOOL`)                   | Switch                 |
| INT, SINT, DINT, LINT, USINT, UINT, UDINT, ULINT (`nINT`, `nSINT`, `nDINT`, `nLINT`, `nUSINT_BYTE`, `nUINT_WORD`, `nUDINT_DWORD`, `nULINT`) | Number |
| REAL, LREAL (`fREAL`, `fLREAL`)  | Number                 |
| STRING (`sSTRING`)               | Text                   |
...
- **TimeSwitch** — scheduling logic handled on the PLC
- **ChargingStation** — complex widget with no standard HA mapping
